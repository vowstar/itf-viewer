...
fn test_full_parsing_workflow() {
    ...
    assert!(stack
        .via_stack
        .get_via_between_layers("poly", "metal1")
        .is_some());
    assert!(stack
        .via_stack
    ...
    assert!(stack
        .via_stack
        .get_via_between_layers("metal2", "metal3")
        .is_some());
}
...
fn test_layer_positioning() {
    ...
    for (i, layer) in stack.layers.iter().enumerate() {
        ...
        if i > 0 {
            ...
            assert!(
                (prev_top - current_bottom).abs() < 1e-10,
...
fn test_via_positioning() {
    ...
    for via in stack.via_stack.iter() {
        ...
        assert!(
            (via.get_bottom_z() - expected_bottom).abs() < 1e-6,
            "Via {} bottom positioning error",
            via.name
        );
        assert!(
            (via.get_top_z() - expected_top).abs() < 1e-6,
            "Via {} top positioning error",
            via.name
        );
    }
}
...
fn test_file_parsing_from_path() {
    ...
    let stack =
        parse_itf_from_file("tests/data/simple_1p3m.itf").expect("Failed to parse from file path");
...
mod renderer_tests {
    ...
    use itf_viewer::renderer::{
        ColorScheme, RectangleShape, StackRenderer, TrapezoidShape, ViewTransform,
    };
...
fn test_all_test_files_parse() {
    ...
    for file_path in &test_files {
        ...
        let stack =
            parse_itf_file(&content).unwrap_or_else(|e| panic!("Failed to parse {file_path}: {e}"));

        // Basic validation
        assert!(
            stack.validate_stack().is_ok(),
            "Stack validation failed for {file_path}"
        );
        assert!(
            stack.get_layer_count() > 0,
            "No layers found in {file_path}"
        );

        let summary = stack.get_process_summary();
        assert!(
            !summary.technology_name.is_empty(),
            "Empty technology name in {file_path}"
        );
        assert!(
            summary.total_height > 0.0,
            "Invalid total height in {file_path}"
        );
    }
}
