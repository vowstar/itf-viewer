...
mod tests {
    ...
    fn test_complete_rendering_pipeline() {
        ...
        assert!(
            shapes.len() > 20,
            "Should generate many shapes for comprehensive stack"
        );
            ...
            match base_name {
                "via_m1_m2" | "via_alt_m1_m2" => {
                    ...
                    assert!(
                        ...
                        via_geom.layer_name,
                        expected_start.min(expected_end),
                        via_geom.z_bottom
                    );
                    assert!(
                        (via_geom.z_top - expected_start.max(expected_end)).abs() < 1e-6,
                        "VIA {} z_top should be {}, but is {}",
                        via_geom.layer_name,
                        expected_start.max(expected_end),
                        via_geom.z_top
                    );
                }
                "via_m2_m3" => {
    ...
    fn test_hit_testing_with_new_features() {
        ...
        let conductor_geom = geometries
            .iter()
            .find(|g| matches!(g.shape, LayerShape::ThreeColumnTrapezoid(_)))
