...
pub fn validate_file<P: AsRef<Path>>(file_path: P) -> Result<FileInfo, FileError> {
    ...
    let size = get_file_size(path).map_err(|e| FileError::ReadError(path.to_path_buf(), e))?;
...
mod tests {
    ...
    fn test_extract_technology_name() {
        ...
        assert_eq!(extract_technology_name("file"), Some("file".to_string()));
    }
    ...
    fn test_find_itf_files() {
        ...
        assert!(
            itf_files[0].file_name().unwrap().to_str().unwrap()
