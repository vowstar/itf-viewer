...
use egui::{Context, CentralPanel, Frame, Sense, Vec2, Pos2, Color32};
...
impl StackViewer {
        ...
        CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                if let Some(stack) = stack {
                    // Render the stack
                    let shapes = self.renderer.render_stack(stack, &self.transform, viewport_rect);
    ...
    fn handle_mouse_input(&mut self, ui: &mut egui::Ui, response: &egui::Response) {
        ...
        if response.hovered() {
            ...
            if scroll_delta.y != 0.0 {
                ...
                let zoom_center = response.interact_pointer_pos()
    ...
    fn handle_keyboard_input(&mut self, ui: &mut egui::Ui) {
        ...
        if input.key_pressed(egui::Key::Plus) || input.key_pressed(egui::Key::Equals) {
            ...
            self.transform.zoom(self.zoom_sensitivity, Pos2::new(center.x, center.y));
        }
        
        if input.key_pressed(egui::Key::Minus) {
            let center = self.transform.viewport_size * 0.5;
            self.transform.zoom(1.0 / self.zoom_sensitivity, Pos2::new(center.x, center.y));
        }
    ...
    fn show_status_overlay(&self, ui: &mut egui::Ui, stack: &ProcessStack, viewport_rect: egui::Rect) {
